...
> **三条核心架构约束（v0.1 即生效）**
>
> 1. **资产不装 NetBird**：所有真实资产（RDS、MySQL、K8s API、Windows Server、IoT 等）位于项目 Agent 后方私网。Agent 是该项目唯一的 NetBird Peer / Routing Peer。
...
每个**项目**部署 1 个（v0.3 起 2+）**YuSui Agent**。它是该项目唯一的 NetBird Peer，把项目私网作为 Network Route 注册到 NetBird，所有进入该项目的流量都从这里穿过。
...
- 项目/Agent 注册时分配 Group、Network Route，运行期基本不动。
...
**YuSui Agent（v0.1 即必需）**
- **NetBird Routing Peer**：通过 Setup Key 加入 NetBird 网络，向 NetBird 注册该项目的私有网段为 Network Route。
- **本地 ACL 引擎**：nftables（v0.1）/ eBPF（v0.3+）执行细粒度策略。NetBird 一条放行只到 "server-peer 可以到 agent"；具体 "可以到哪台资产的哪个端口" 全由 Agent nftables 控制。
- **资产发现**：根据配置的网段做被动发现（ARP/扫描），上报候选资产给 Server。MVP 仅做"已知 IP 列表上报"，不做主动扫描。
- **运行环境**：单二进制 + systemd / Docker，最小依赖（NetBird client + nftables）。可装在跳板 VM、K8s Node、甚至 OpenWrt 旁路设备上。
...
[运维] 点"打开终端" → 浏览器 WebSocket ─attach─▶ Web Shell  │
   │                                              │           │
   │                              Server 建 SSH ▼            │
   │             流量：Server-Peer ─P2P─▶ Agent ─nft─▶ mysql │
   │                                                          │
...
- **真正的 ACL 在 Agent**：nftables set 元素绑 TTL，源 IP 为 server-peer 的内网 IP（v0.1 1 个 / v0.3 多副本一组），目标精确到资产 IP:port。这就是唯一的"谁能进哪里"的事实表。
...
| **YuSui Agent v0.1** | 单二进制，NetBird Setup Key 注册，gRPC 上行，nftables TTL set 下发 | **2 周** |
...
- **Agent 双活**：每项目 ≥2 Agent，NetBird Routing Peer HA Group，Server 多副本规则双写
