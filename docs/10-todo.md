...
| 项 | 状态 | 现状 / 不做会怎样 | 规划 | 关联 |
|---|---|---|---|---|
| ~~**NetBird daemon 管理(`overlay.Netbird`)**~~ | ✅ 已落地 + 全 overlay e2e 验证(gRPC daemon API 待补) | `overlay.New(Config{Kind:"netbird"})` 已实现:设了 setup key 就 `netbird up`,然后从 WireGuard 接口(默认 `wt0`)自动发现 overlay IP 作 forwarder `ListenHost()`、上报 connected/down。**`scripts/e2e-overlay.sh` 全链路验证**:server + agent 各做一个真实 NetBird peer(yusui 进程共享 netbird daemon 容器 netns),server 经**真实 WireGuard overlay** 拨到 agent 转发器的 **overlay IP** → 中继到资产,危险命令拦截生效——6/6 断言过。v1 用 `netbird` CLI(本身就是 daemon 本地 API 的客户端);draft10 偏好的「直连 daemon 本地 gRPC API」是后续细化。 | draft10 收尾 | `agent/internal/overlay/overlay.go`、[deploy/netbird](../deploy/netbird)、`scripts/e2e-overlay.sh` |
