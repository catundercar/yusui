...
if [ "${1:-}" = "--vms" ] || [ "${2:-}" = "--vms" ]; then
  echo "deleting VMs…"
  for vm in yusui-agent yusui-asset1 yusui-asset2; do orb delete -f "$vm" >/dev/null 2>&1; done
fi
