...
| 2026-06-10 | v0.1-draft12 | 新增 **[docs/11 Agent 注册/daemon 与客户端](docs/11-agent-enrollment.md)**(设计):把项目 agent 做成 NetBird 式本地 daemon + 多客户端(CLI 先做,GUI 延后);**零接触自动注册 + 人工审核准入 + Server 下发 NetBird setup key 自动入网**(认证先不做 = 无密码学认证、以人工审核当临时闸,记 docs/10 债)。新增正交的 `agents.enrollment`(pending/approved/rejected)状态 + `netbird_setup_key`;`Register` 改为未知 agent 自动建 pending 行(不再硬要预建 primary);admin `approve/reject` API。落地 docs/10 门控的 `overlay.Netbird`。分 P1–P6 实现。纯设计文档。 |
